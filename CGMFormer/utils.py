...
from .modeling_bert import BertForMaskedLM, BertForTokenClassification, BertForSequenceClassification
...
def compute_metrics_V3(pred, id2token, unit="mmol/L"):
    """
    支持 mmol/L 与 mg/dL 两种量纲
    ------------------------------------------------------------------
    unit = "mmol/L" →  <=3.9, 3.9~10,  >10,  tol=0.3
    unit = "mg/dL"  →  <=70 , 70~180,  >180, tol=5.4
    ------------------------------------------------------------------
    """
    # ---------- 0. 单位参数 ----------
    unit = unit.lower()
    if unit in ("mg/dl", "mg_dl", "mg"):
        low_thr, high_thr, tol = 70.0, 180.0, 5.4
        mae_keys = ("mae_below_70", "mae_70_to_180", "mae_above_180")
    else:                                              # 默认 mmol/L
        low_thr, high_thr, tol = 3.9, 10.0, 0.3
        mae_keys = ("mae_below_3.9", "mae_3.9_to_10", "mae_above_10")
    ...
    mask = mlm_labels != -100
    mlm_acc = (mlm_preds == mlm_labels)[mask].mean() if mask.any() else 0.0
    mlm_total = mask.sum().item()

    # ---------- 2. id → 数值 ----------
    def safe_lookup(idx):
        try:
            return float(id2token[idx])
        except ValueError:
            return np.nan

    label_vals = np.array([safe_lookup(i) for i in mlm_labels[mask]])
    pred_vals  = np.array([safe_lookup(i) for i in mlm_preds [mask]])

    # ---------- 3. ±tol 准确 ----------
    acc_tol = np.logical_and(
        pred_vals >= label_vals - tol,
        pred_vals <= label_vals + tol
    ).mean() if label_vals.size else 0.0

    # ---------- 4. MAE 分段 ----------
    total_cnt = [0, 0, 0]
    cum_err   = [0.0, 0.0, 0.0]
    for p, l in zip(pred_vals, label_vals):
    ...
    mae = [cum_err[i] / total_cnt[i] if total_cnt[i] else 0.0 for i in range(3)]

    # ---------- 5. 插值评估 ----------


    # ---------- 6. 相关性 ----------
    pearson_corr_model, _ = pearsonr(pred_vals, label_vals)
    r_squared_model       = r2_score(label_vals, pred_vals)

    return {
        "accuracy": mlm_acc,
        "accuracy_±tol": acc_tol,
        mae_keys[0]: mae[0],
        mae_keys[1]: mae[1],
        mae_keys[2]: mae[2],
        "model_pearson_correlation": pearson_corr_model,
        "model_r_squared": r_squared_model,
    }

def compute_metrics_V2(pred):
    mlm_labels = pred.label_ids # <class 'numpy.ndarray'>
    mlm_preds = pred.predictions.argmax(-1)
    inputs = pred.inputs # "include_inputs_for_metrics": True

    mlm_acc = np.equal(mlm_preds, mlm_labels)
    mask = np.not_equal(mlm_labels, -100)

    # print(f"mlm_labels[mask]: {mlm_labels[mask]}")

    mlm_acc = mlm_acc[mask]
    mlm_correct = mlm_acc.sum().item()
    mlm_total = mask.sum().item()

    mlm_acc = float(mlm_correct) / mlm_total if mlm_total > 0 else 0.0


    special_glucose_value = -1

    # 基于插值
    input_values = []
    # Converting input IDs back to actual values and mark the masked positions
    for input_seq in inputs:
        # print(f"input_seq: {input_seq}")
        seq_values = []
        for input_id in input_seq:
            try:
                # print(f"input_id: {input_id}")
                value = float(id2token[input_id])
                seq_values.append(value)
            except ValueError:
                seq_values.append(-1)  # Handle missing or incorrect tokens
        input_values.append(seq_values)

    # interpolation_preds = input_values.copy()
    interpolation_preds = [row[:] for row in input_values] # make sure deep copy
    for seq_idx, sequence in enumerate(input_values):
        for idx, value in enumerate(sequence):
            if value == -1:
                left_idx = idx - 1
                while left_idx >= 0 and sequence[left_idx] == -1:
                    left_idx -= 1

                right_idx = idx + 1
                while right_idx < len(sequence) and sequence[right_idx] == -1:
                    right_idx += 1

                left_val = input_values[seq_idx][left_idx] if left_idx >= 0 else 0.0  # use 0 if no left neighbor
                right_val = input_values[seq_idx][right_idx] if right_idx < len(sequence) else 0.0  # use 0 if no right neighbor

                interp_val = round(((left_val + right_val) / 2.0), 1)
                interpolation_preds[seq_idx][idx] = interp_val


    interpolation_preds = np.array(interpolation_preds)

    interpolated_values_masked = interpolation_preds[mask]

    # 计算MAE
    range_mlm_preds = []
    range_mlm_labels = []
    for pred, label in zip(mlm_preds[mask], mlm_labels[mask]):
        try:
            # print(f"pred: {pred}")
            # print(f"pred.item(): {pred.item()}")
            pred_glucose_value = float(id2token[pred])
            range_mlm_preds.append(pred_glucose_value)
        except ValueError:
            pred_glucose_value = -1
            range_mlm_preds.append(-1)

        try:
            label_glucose_value = float(id2token[label])
        except ValueError:
            label_glucose_value = -1
        range_mlm_labels.append(label_glucose_value)

    range_mlm_preds = np.array(range_mlm_preds)
    range_mlm_labels = np.array(range_mlm_labels)

    acc_plus_minus_0_3 = np.logical_and(range_mlm_labels - 0.3 <= range_mlm_preds,
                                        range_mlm_preds <= range_mlm_labels + 0.3)
    acc_plus_minus_0_3_correct = acc_plus_minus_0_3.sum()
    acc_plus_minus_0_3 = float(acc_plus_minus_0_3_correct) / mlm_total if mlm_total > 0 else 0.0


    total_counts_mae = [0, 0, 0]
    cumulative_error = [0.0, 0.0, 0.0]

    for pred, label in zip(range_mlm_preds, range_mlm_labels):
        diff = abs(pred - label)
        if label <= 3.9:
            total_counts_mae[0] += 1
            cumulative_error[0] += diff
        elif 3.9 < label <= 10:
            total_counts_mae[1] += 1
            cumulative_error[1] += diff
        else:
            total_counts_mae[2] += 1
            cumulative_error[2] += diff

    mae = [cumulative_error[i] / total_counts_mae[i] if total_counts_mae[i] > 0 else 0.0 for i in range(3)]

    # 计算插值方法的MAE和准确性
    interp_mae = np.mean(np.abs(interpolated_values_masked - range_mlm_labels))
    interp_accuracy_03 = np.mean((interpolated_values_masked >= range_mlm_labels - 0.3) & (interpolated_values_masked <= range_mlm_labels + 0.3))

    # 分组计算MAE
    interp_mae_below_39 = np.mean([abs(p - l) for p, l in zip(interpolated_values_masked, range_mlm_labels) if l <= 3.9])
    interp_mae_39_to_10 = np.mean([abs(p - l) for p, l in zip(interpolated_values_masked, range_mlm_labels) if 3.9 < l <= 10])
    interp_mae_above_10 = np.mean([abs(p - l) for p, l in zip(interpolated_values_masked, range_mlm_labels) if l > 10])

    # 相关性指标
    pearson_corr_interp, _ = pearsonr(interpolated_values_masked, range_mlm_labels)
    r_squared_interp = r2_score(range_mlm_labels, interpolated_values_masked)

    pearson_corr_model, _ = pearsonr(range_mlm_preds, range_mlm_labels)
    r_squared_model = r2_score(range_mlm_labels, range_mlm_preds)

    return {
        'accuracy': mlm_acc,
        'accuracy_0.3': acc_plus_minus_0_3,
        'mae_below_3.9': mae[0],
        'mae_3.9_to_10': mae[1],
        'mae_above_10': mae[2],
        'model_pearson_correlation': pearson_corr_model,
        ...
        'interp_accuracy_+-0.3': interp_accuracy_03,
        'interp_mae_below_3.9': interp_mae_below_39,
        'interp_mae_3.9_to_10': interp_mae_39_to_10,
        'interp_mae_above_10': interp_mae_above_10,
        'interp_pearson_correlation': pearson_corr_interp,
        'interp_r_squared': r_squared_interp,
    }



# load data and filter by defined criteria
def load_and_filter(filter_data, filter_target, nproc, input_data_file):
    ...
    # # 过滤掉具有NaN标签的训练样本
    # if filter_target != "types" and filter_target != "type":
    #     trainset = data_shuffled.filter(lambda data_shuffled: not (data_shuffled[filter_target] is None or math.isnan(data_shuffled[filter_target])))
    # trainset = data_shuffled
...
def load_model(model_type, num_classes, model_directory):
    if model_type == "Pretrained":
        ...
        model = BertForSequenceClassification.from_pretrained(model_directory, 
                                                ...
                                                output_hidden_states=True, 
                                                output_attentions=False,
                                                ignore_mismatched_sizes=True) # for Colas dataset
...
def get_model_input_size(model):
    # return int(re.split("\(|,",str(model.bert.embeddings.position_embeddings))[1]) # 替换为SinusoidalPositionEmbedding()后报错
    return model.bert.config.max_position_embeddings
...
def gen_attention_mask(minibatch_encoding, max_len = None):
    ...
    original_lens = [289 for _ in minibatch_encoding]
...
def mean_nonpadding_embs(embs, original_lens):
    ...
    len_seq = len(embs)
    original_lens = torch.ones(len_seq, device='cuda') * 289
